...
class JuliaBackend(BackendProtocol):
    ...
    def __init__(self, base_url: str = "http://localhost:9000"):
        ...
        try:
            import requests  # lazy import guard
...
class DAGOrchestrator:
    def __init__(self):
        ...
        self._in_degree: Dict[str, int] = {}    # in-degree of nodes
...
# ==============================
# Surrogate utilities (inline for DAG node)
# ==============================
...
def _ridge_closed_form(Phi: np.ndarray, Y: np.ndarray, lam: float = 1e-6) -> np.ndarray:
    PtP = Phi.T @ Phi
    PtY = Phi.T @ Y
...
def build_orchestrator(backend: BackendProtocol) -> SerializableDAGOrchestrator:
    ...
    # --- Polynomial surrogate fit node ---
    def poly_surrogate_node(raw_matrix: list, test_matrix: list,
                            degree: int = 2, lam: float = 1e-6,
                            max_input_cols: int = 8, max_target_cols: int = 12) -> dict:
        X = np.asarray(raw_matrix, dtype=float)[:, :max_input_cols]
        Y = np.asarray(test_matrix, dtype=float)[:, :max_target_cols]
        n = min(X.shape[0], Y.shape[0])
        X, Y = X[:n], Y[:n]
        mask = np.isfinite(X).all(axis=1) & np.isfinite(Y).all(axis=1)
        X, Y = X[mask], Y[mask]
        if X.size == 0 or Y.size == 0:
            raise ValueError("poly_surrogate_node: no valid rows after NaN filtering")
        Phi = _poly_features(X, degree=degree)
        B = _ridge_closed_form(Phi, Y, lam=lam)
        pred = Phi @ B
        ...
        return {
            ...
            "rmse_first10": [float(x) for x in rmse[:10]],
        }

    # entropy: list[chunk] -> list[metrics]
    ...
    # projection: stacks chunks, returns dict with projected + original_shape
    ...
    # ghost check: consumes projection dict
    ...
    # adaptive optimization: accepts ghost feedback
    ...
    adaptive_optimization_node.accepts_ghost_feedback = True  # type: ignore[attr-defined]

    # --- Register nodes (any order allowed) ---
    orch.add_node("poly_surrogate_fit",
                  ["raw_matrix", "test_matrix", "degree", "lambda"], poly_surrogate_node)
...
if __name__ == "__main__":
    ...
    # simple matrices for surrogate demo
    raw_matrix = [[1,2,3],[2,3,4],[3,4,5],[4,5,6]]
    test_matrix = [[1.5, 2.0],[2.0, 2.5],[2.5, 3.0],[3.0, 3.5]]
    ...
    for k, v in result.items():
        if k in ("chunks", "degree", "lambda", "raw_matrix", "test_matrix"):
            continue
        print(f"\n[{k}]")
